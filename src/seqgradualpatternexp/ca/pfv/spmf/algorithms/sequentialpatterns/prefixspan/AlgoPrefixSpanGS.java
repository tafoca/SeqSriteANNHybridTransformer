...
public class AlgoPrefixSpanGS {
	...
	private void prefixSpan(SequenceDatabase sequenceDatabase, String outputFilePath) throws IOException {
		...
		Map<String, List<Integer>> mapSequenceID = null;
		Map<String, ItemGIDsSGs> mapSequenceID2 = null;
		if (AppConstants.SAVE_MEASURE) {
			...
			mapSequenceID2 = findSequencesContainingItems2();
		} else {
			// ============== CALCULATE FREQUENCY OF SINGLE ITEMS =============
			// We have to scan the database to find all frequent sequential patterns of size
			// 1.
		...
		if (containsItemsetsWithMultipleItems) {
			if (AppConstants.SAVE_MEASURE) {
				prefixspanWithMultipleItems1(mapSequenceID2);
			} else {
			...
			if (AppConstants.SAVE_MEASURE) {
				prefixspanWithSingleItems1(mapSequenceID2);
			} else {
		...
		displayMapBySubTree();
		saveInFileMapBySubTree();
		/*
		 * elagageByMapAllIntrsSubTree(); System.out.
		 * println("+++++++++++++++++++++++++++ ( after 2nd prunning)+++++++++++++++++++++++++++"
		 * ); displayMapBySubTree();
		 */
	...
	boolean checkEqs(String[] list1, String[] list2) {
		...
		/*
		 * displayTab(list1); System.out.print(" C "); displayTab(list2);
	...
	public void saveInFileMapBySubTree() throws IOException {
		String outputFilePath = "outputmax.txt";
		BufferedWriter writer = new BufferedWriter(new FileWriter(outputFilePath));
		for (Entry<Integer, List<String>> entry : mapSeqByPartSubTree.entrySet()) {
			Integer key = entry.getKey();
			List<String> val = entry.getValue();
			for (Iterator iterator = val.iterator(); iterator.hasNext();) {
				String _item = (String) iterator.next();
				writer.write(_item.toString());
				writer.newLine();
				System.out.println("\n" + _item);
			}
			System.out.println("=================================================");
	...
	public void elagageByMapBySubTree(int level) {
		...
		List<String> levelToTraited = mapSeqByPartSubTree.get(level);// les sequentiel graduel frequent d'une memme
																		// sous-abre
		...
		// boolean checkEqs = checkEqs(split_e1, split_e2);
		/*
		 * int i = 0; for (String e : levelToTraited) { System.out.println(i+": \n >  "
		 * + e); /* if (i == 4) { split_e1 = e.split("-1"); } else if (i == 3) {
		 * split_e2 = e.split("-1"); } else { split_e = e.split("-1"); } for (String e2
		 * : levelToTraited) { boolean checkEqs = checkEqs(split_e1, split_e2); }
		 * 
		 * i++; }
		 */
		// boolean checkEqs = checkEqs(split_e1, split_e2);
	 ...
	 */
	private void prefixspanWithSingleItems(Map<String, List<Integer>> mapSequenceID) throws IOException {
		...
		for (int i = 0; i < sequenceDatabase.size(); i++) {
			...
			for (int j = 0; j < sequence.length; j++) {
				...
				if (AppConstants.SAVE_MEASURE) {
					...
					if (token.charAt(0) != '-') {
						String[] eList = token.split(";");
						boolean isFrequent = mapSequenceID.get(eList[0]).size() >= minsuppAbsolute;

						// if the item is frequent
						if (isFrequent) {
							// copy the item to the current position
							sequence[currentPosition] = eList[0];
				...
				} else {// gestion cas sans sg dans la bd
					// if it is an item because -1 and -2 are separators
					if (token.charAt(0) != '-') {
	...
	private void prefixspanWithMultipleItems1(Map<String, ItemGIDsSGs> mapSequenceID2) {

	}

	private void prefixspanWithSingleItems1(Map<String, ItemGIDsSGs> mapSequenceID2) throws IOException {
		// =============== REMOVE INFREQUENT ITEMS ========================
		// We scan the database to remove infrequent items and resize sequences after
		// removal
		// for each sequence in the current database
		for (int i = 0; i < sequenceDatabase.size(); i++) {
			...
			for (int j = 0; j < sequence.length; j++) {
				...
				if ('-' != token.charAt(0)) {
					String[] eList = token.split(";");
					boolean isFrequent = mapSequenceID2.get(eList[0]).sequenceIDs.size() >= minsuppAbsolute;
					...
					if (currentPosition > 0) {
						...
						sequenceDatabase.getSequences().set(i, newSequence);
						// continue to next sequence
						/*System.out.println(i+ ", "+ j+" >> n taille: "+ sequenceDatabase.size());
						continue;*/
						if(i>=sequenceDatabase.size()-1) {
							System.out.println(">> 2 taille: "+ sequenceDatabase.size());
							break;
						}else {
							System.out.println("> 1 taille: "+ sequenceDatabase.size());
							continue;
						}
					} else {
						// if the sequence is empty, delete this sequence by replacing it with null
		...
		// ================================
		// For each frequent item
		// TODO 3 ajouter initialisation et construction des # niveau de la map
		// mapSeqByPartSubTree
		for (Entry<String, ItemGIDsSGs> entry : mapSequenceID2.entrySet()) {
			int support = entry.getValue().sequenceIDs.size();
			// if the item is frequent (has a support >= minsup)
			if (support >= minsuppAbsolute) {
				mapSeqByPartSubTree.put(level, new ArrayList<>());
				String item = entry.getKey();
	...
	private void savePattern(String item, int support, ItemGIDsSGs gIDsSGs) throws IOException {
		// increase the number of pattern found for statistics purposes
		patternCount++;
		...
		if (writer != null) {
			...
			r.append(item);
			r.append(" -1 #SUP: ");
			r.append((float) support / (float) sequenceCount);
			r.append(";");
			float sgm = 0;
			for (int i = 0; i < gIDsSGs.SGByIDs.size(); i++) {
				sgm += gIDsSGs.SGByIDs.get(i);
			}
			sgm /= sequenceCount;
			r.append(sgm);

			if (sgm >= AppConstants.THREHOLDWGS) {
				mapSeqByPartSubTree.get(level).add(r.toString());
				// write the string to the file
				writer.write(r.toString());
				// start a new line
				...
				System.out.println("\n infrequent category 2:: " + r.toString());
			}

			if (showSequenceIdentifiers) {
				r.append(" #SID: ");
				for (Integer sid : gIDsSGs.sequenceIDs) {
					r.append(sid);
					r.append(" ");
				}
			}
			...
			SequentialPattern pattern = new SequentialPattern();
			pattern.addItemset(new Itemset(item));
			pattern.setSequenceIDs(gIDsSGs.sequenceIDs);
			patterns.addSequence(pattern, 1);
		}
	}
	...
	private void savePattern(int lastBufferPosition, List<PseudoSequence> pseudoSequences) throws IOException {
		...
		if (writer != null) {
			...
			// -------------------------------------
			r.append("#SUP: ");
			if (mapSeqByPartSubTree.get(level).size() >= 1) {
				String previousSentialpattern = mapSeqByPartSubTree.get(level)
						.get(mapSeqByPartSubTree.get(level).size() - 1);
				String[] sgmPreced = previousSentialpattern.split(";");
				Float moy1 = Float.parseFloat(sgmPreced[1]);
				r.append((float) pseudoSequences.size() / (float) sequenceCount);
				r.append(";");
				if (AppConstants.SAVE_MEASURE) {
					float sgm = 0;
					for (PseudoSequence sequence : pseudoSequences) {
						String[] seq = sequenceDatabase.getSequences().get(sequence.sequenceID);
						String name = seq[sequence.indexFirstItem - 1];
						Float val = Float.parseFloat(name.split(";")[1]);
						sgm += val;
					}
					sgm /= (float) sequenceCount;
					sgm += (float) moy1;
					sgm /= (float) 2;
					r.append(sgm);
...
//			System.out.println(pattern);
	...
	private Map<String, List<Integer>> findSequencesContainingItems() {
		...
		Map<String, List<Integer>> mapSequenceID = new HashMap<>();// Key : item Value : a set of sequence IDs
		...
		for (int i = 0; i < sequenceDatabase.size(); i++) {
			...
			for (String token : sequence) {
				...
				if ('-' != token.charAt(0)) {
					...
					// extraire item graduel du token sep= ;
					if (!AppConstants.SAVE_MEASURE) {
...
//		System.out.println(mapSequenceID);
	 ...
	 * @return Map of items to sequence IDs that contains each item
	 */
	private Map<String, ItemGIDsSGs> findSequencesContainingItems2() {
		// number of items in the current itemset
		int itemCountInCurrentItemset;
		// We use a map to store the sequence IDs where an item appear
		Map<String, ItemGIDsSGs> mapSequenceID1 = new HashMap<>(); // Key : item Value : a ItemGIDsSGs elt
		...
		for (int i = 0; i < sequenceDatabase.size(); i++) {
			...
			for (String token : sequence) {
				...
				if ('-' != token.charAt(0)) {
					...
					// extraire item graduel du token sep= ;
					if (AppConstants.SAVE_MEASURE) {
						String[] elist = token.split(AppConstants.SEP_MEASURE);
						// get the set of sequence IDs for this item until now
						ItemGIDsSGs sequenceIDsGpSups = mapSequenceID1.get(elist[0]);
						if (sequenceIDsGpSups == null) {
							// if the set does not exist, create one
							sequenceIDsGpSups = new ItemGIDsSGs(elist[0]);
							mapSequenceID1.put(elist[0], sequenceIDsGpSups);
						}
						// add the sequence ID to the
						// set of sequences IDs of this item
						// if it is not already there
						if (sequenceIDsGpSups.getSequenceIDs().size() == 0 || sequenceIDsGpSups.getSequenceIDs()
								.get(sequenceIDsGpSups.getSequenceIDs().size() - 1) != i) {
							sequenceIDsGpSups.getSequenceIDs().add(i);
							sequenceIDsGpSups.getSGByIDs().add(Float.parseFloat(elist[1]));// get the first value of
																							// support
						}
						itemCountInCurrentItemset++;
						// if this itemset contains more than 1 item, we will remember that this
...
//		System.out.println(mapSequenceID);
	...
	/**
	 *
	 * case of function taking account support gradual Create a projected database
	 * by pseudo-projection with the initial database and a given item. (this have a
	 * support SG )
	 *
	 * @param item
	 * @param sequenceIDsSGs
	 * @return
	 */
	private List<PseudoSequence> buildProjectedDatabaseFirstTimeMultipleItems(String item, ItemGIDsSGs sequenceIDsSGs) {
		// We create a new projected database
		List<PseudoSequence> projectedDatabase = new ArrayList<PseudoSequence>();

		// for each sequence that contains the current item
		loopSeq: for (int sequenceID : sequenceIDsSGs.sequenceIDs) {
			String[] sequence = sequenceDatabase.getSequences().get(sequenceID);
...
//		System.out.println("---");
		...
		Map<String, List<PseudoSequence>> itemsPseudoSequences = findAllFrequentPairsSingleItems(database,
				lastBufferPosition);
...
//		System.out.println("DEBUG");
	 ...
	 * @throws IOException exception if there is an error writing to the output file
	 */
	private void recursion(String[] patternBuffer, List<PseudoSequence> database, int k, int lastBufferPosition)
			throws IOException {
//		for(int i=0; i<= lastBufferPosition; i++){
//			System.out.print(patternBuffer[i] + " ");
//		}
...
//		System.out.println("---");
		...
		for (Entry<Pair, Pair> entry : mapsPairs.mapPairs.entrySet()) {
			...
			if (pair.getCount() >= minsuppAbsolute) {
				...
				newBuferPosition++;
				if (AppConstants.SAVE_MEASURE) {
					patternBuffer[newBuferPosition] = pair.item.split(";")[0];
					// save the pattern
					savePattern(newBuferPosition, pair.getPseudoSequences());
				} else {
					patternBuffer[newBuferPosition] = pair.item;
	...
	public void displayTraceExecprojectedDB(int lastBufferPosition, String[] patternBuffer1,
			List<PseudoSequence> database, MapFrequentPairs mapsPairs) {
...
// where it appears in the projected database).
	...
	/**
	 * Method to find all frequent gradual items in a projected sequence database
	 *
	 ...
	 *         IDs where it occurs.
	 */
	protected Map<String, List<PseudoSequence>> findAllFrequentPairsSingleItems(List<PseudoSequence> sequences,
	...
	protected MapFrequentPairs findAllFrequentPairs(List<PseudoSequence> sequences, int lastBufferPosition) {
		...
		while (lastBufferPosition > 0) {
			...
			if (firstPositionOfLastItemsetInBuffer < 0
					|| "-1".equals(patternBuffer[firstPositionOfLastItemsetInBuffer])) {
				firstPositionOfLastItemsetInBuffer++;
				break;
			}
		}
